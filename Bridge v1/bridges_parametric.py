...
class Bridge:
    ...
    class TensionMember:
        def __init__(self, ultimate_tensile_str, cross_sec_area, M_max=0, depth=None, length=None):
            """Initialize tension member with optional bending moment, depth, and length."""
    ...
    class CompressionMember:
        def __init__(self, length, thickness, depth, K, E, sigma_ult):
            """Initialize compression member with geometry, buckling factor K, E, and sigma_ult."""
            self.length = length
            ...
            self.area = self.A
            self.I_in_plane = thickness * depth**3 / 12
            self.I_out_of_plane = depth * thickness**3 / 12
            self.c = depth / 2
        ...
        def combined_stress_failure_ratio(self, F_axial, M_max=0, is_in_plane=True):
            """Stress ratio (< 1 is safe). Combined axial + bending."""
        ...
        def find_euler_buckle_F(self, axial_lambda, moment_lambda=None, is_in_plane=True):
            """Find critical applied load F that causes Euler buckling (with optional moment interaction)."""
            from scipy.optimize import fsolve
        ...
        def find_material_strength_F(self, axial_lambda, moment_lambda=None, is_in_plane=True):
            """Find critical applied load F that causes material strength failure."""
            from scipy.optimize import fsolve
        ...
        def find_buckle_F(self, axial_lambda, moment_lambda=None, is_in_plane=True):
            """Find critical F: min of Euler buckling and material strength limits."""
