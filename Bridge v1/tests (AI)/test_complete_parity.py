"""Complete parity check between ``bridges_parametric`` and ``pratt_torch``.

Unlike the unittest-based parity tests, this module provides a standalone
``test_parity`` function that prints a detailed human-readable table showing
the parametric and PyTorch failure loads side-by-side together with their
percentage differences.  It is intended to be run interactively when
debugging numerical discrepancies.

Three representative configurations are tested when the script is executed
directly:

- **Standard** — typical geometry, moderate strengths.
- **Slender**   — thin members where Euler buckling dominates.
- **Stocky**    — thick, short members where material yield dominates.
"""
...
def test_parity(params_dict, test_name="Default", tolerance_pct=1.0):
    """Compare parametric and PyTorch failure loads for a given bridge configuration.

    Prints a formatted table with one row per failure mode showing::

        Mode | Parametric (N) | Torch (N) | Diff % | Status

    Statuses are:
    - ``OK``      — relative difference is below *tolerance_pct*.
    - ``FAIL``    — relative difference exceeds *tolerance_pct*.
    - ``SIGN``    — the two implementations disagree on sign.
    - ``MISSING`` — mode present in parametric but absent from torch.
    - ``EXTRA``   — mode present in torch but absent from parametric.

    Args:
        params_dict:    Full parameter dictionary accepted by both
                        ``max_load_torch`` and ``OnePanelPratt2D``.
                        May include an optional ``K`` key (effective-length
                        factor) which is stripped before passing to the
                        parametric model.
        test_name:      Label shown in the printed header.
        tolerance_pct:  Maximum acceptable relative difference (percent).

    Returns:
        bool: True if all modes matched within tolerance, False otherwise.
    """
