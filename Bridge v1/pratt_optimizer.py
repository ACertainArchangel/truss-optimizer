"""
Bridge optimizer: trains bridge design parameters to maximize critical load.
Supports fixed constraints (material properties, span length, etc.) and trainable parameters (cross-sections, geometry).
"""
...
class BridgeOptimizer:
    ...
    def __init__(
        ...
        """Initialize optimizer with initial params, fixed/trainable flags, bounds, and learning rate."""
    ...
    def loss_function(self) -> torch.Tensor:
        """Compute loss based on ratio_mode: None=-load, weight_per_load=W/F, max_load_per_weight=-F/W."""
    ...
    def train(
        ...
        """Train for num_iterations steps. Returns (initial_params_dict, final_params_dict)."""
        ...
        """
        final_params_dict = {
            name: tensor.detach().item() for name, tensor in self.trainable_params.items()
        }
        return self.initial_params_dict, final_params_dict
...
def make_and_train_pratt(
    ...
    """Create and train a Pratt truss bridge optimizer, returning initial and final param dicts."""
    ...
    vprint("=" * 70)
    flattness_corrector = flat_factor
    param_bounds = {
        ...
        "incline_thickness": (0.005, inches_to_meters(0.5)),
        "diagonal_thickness": (0.005, inches_to_meters(0.5)),
        "mid_vert_thickness": (0.005, inches_to_meters(0.5)),
        "side_vert_thickness": (0.005, inches_to_meters(0.5)),
        "top_thickness": (0.005, inches_to_meters(0.5)),
        "bottom_thickness": (0.005, inches_to_meters(0.5)),
        "incline_depth": (0.005, inches_to_meters(0.5)),
        "diagonal_depth": (0.005, inches_to_meters(0.5)),
        "mid_vert_depth": (0.005, inches_to_meters(0.5)),
        "side_vert_depth": (0.005, inches_to_meters(0.5)),
        "top_depth": (0.005, inches_to_meters(0.5)),
        "bottom_depth": (0.005, inches_to_meters(0.5)),
    }
