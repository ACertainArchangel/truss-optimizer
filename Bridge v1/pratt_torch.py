...
def softmin_torch(values, softness=1e-3):
    """Differentiable soft-min via log-sum-exp. Filters NaN/inf."""
...
def max_load_torch(
    ...
    """Compute differentiable max applied load for a OnePanelPratt2D truss (all failure modes)."""
    ...
    I_incline_in = incline_thickness * incline_depth**3 / 12.0
    I_diagonal_in = diagonal_thickness * diagonal_depth**3 / 12.0
    I_mid_in = mid_vert_thickness * mid_vert_depth**3 / 12.0
    I_side_in = side_vert_thickness * side_vert_depth**3 / 12.0
    I_top_in = top_thickness * top_depth**3 / 12.0
    I_bottom_in = bottom_thickness * bottom_depth**3 / 12.0
    I_incline_out = incline_depth * incline_thickness**3 / 12.0
    I_diagonal_out = diagonal_depth * diagonal_thickness**3 / 12.0
    I_mid_out = mid_vert_depth * mid_vert_thickness**3 / 12.0
    I_side_out = side_vert_depth * side_vert_thickness**3 / 12.0
    I_top_out = top_depth * top_thickness**3 / 12.0
    I_bottom_out = bottom_depth * bottom_thickness**3 / 12.0
    ...
    def euler_buckling_Fcrit(E, I, K, L, a_coeff, area, b_coeff=0, c=0):
        """
        Euler buckling with moment interaction.
        Without moments: F_axial <= F_cr
        With moments: F_axial/F_cr + M*c*A/(F_cr*I) <= 1
        Since F_axial = a*F and M = b*F:
        a*F/F_cr + b*F*c*A/(F_cr*I) <= 1
        F*(a/F_cr + b*c*A/(F_cr*I)) <= 1
        F <= 1 / (a/F_cr + b*c*A/(F_cr*I))
        F <= F_cr / (a + b*c*A/I)
        """
...
def compute_volume_torch(
    ...
    """Compute total truss volume (differentiable)."""
...
def compute_weight_torch(
    ...
    """Compute truss weight as volume * density * g (differentiable)."""
