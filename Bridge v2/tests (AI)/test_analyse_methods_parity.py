"""Test that analyse scripts compute the minimum failure load across all modes per member."""
...
class TestAnalyseMethodsParity(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.default_bridge = OnePanelPratt2D(
            ...
            height=2,
            length=10,
            incline_thickness=0.05,
            diagonal_thickness=0.05,
            mid_vert_thickness=0.05,
            side_vert_thickness=0.05,
            top_thickness=0.05,
        ...
        self.custom_bridge = OnePanelPratt2D(
            ...
            incline_depth=0.015,
            diagonal_depth=0.015,
            mid_vert_depth=0.015,
            side_vert_depth=0.015,
            top_depth=0.015,
            bottom_depth=0.015,
            E=200e9,
            sigma_compression=250e6,
    ...
    def test_analyse_scripts_compute_minimum_of_all_modes(self):
        """Verify analyse scripts return the minimum failure load across all modes per member."""
    ...
    def test_pytorch_parity_with_class_method(self):
        ...
        params_torch = {
            ...
            "incline_thickness": torch.tensor(0.05, dtype=torch.float64),
            "diagonal_thickness": torch.tensor(0.05, dtype=torch.float64),
            "mid_vert_thickness": torch.tensor(0.05, dtype=torch.float64),
            "side_vert_thickness": torch.tensor(0.05, dtype=torch.float64),
            "top_thickness": torch.tensor(0.05, dtype=torch.float64),
            "bottom_thickness": torch.tensor(0.05, dtype=torch.float64),
            "incline_depth": torch.tensor(0.02, dtype=torch.float64),
            "diagonal_depth": torch.tensor(0.02, dtype=torch.float64),
            "mid_vert_depth": torch.tensor(0.02, dtype=torch.float64),
            "side_vert_depth": torch.tensor(0.02, dtype=torch.float64),
            "top_depth": torch.tensor(0.02, dtype=torch.float64),
            "bottom_depth": torch.tensor(0.02, dtype=torch.float64),
            "E": 200e9,
            "sigma_compression": 250e6,
            "sigma_tension": 400e6,
        }
