...
from truss_optimizer.core.members import CompressionMember, MemberGeometry, TensionMember
...
class TestMaterials:
    ...
    def test_custom_material(self):
        ...
        custom = materials.Custom(
            E=100e9, sigma_compression=50e6, sigma_tension=60e6, density=1000, name="Test Material"
        )
...
class TestMembers:
    ...
    def test_compression_member_buckling(self):
        ...
        geom = MemberGeometry(length=1.0, thickness=0.01, depth=0.02)
        member = CompressionMember(sigma_compression=20e6, E=200e9, K=1.0, geometry=geom)
...
class TestTorchPhysics:
    ...
    def test_volume_finite(self, params):
        ...
        geom_params = {
            k: v for k, v in params.items() if k not in ("E", "sigma_compression", "sigma_tension")
        }
...
class TestOptimizer:
    ...
    def test_optimization_improves(self):
        ...
        optimizer = BridgeOptimizer(bridge=bridge, objective="load_to_weight", learning_rate=0.01)
