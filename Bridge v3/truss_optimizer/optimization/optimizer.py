...
class BridgeOptimizer:
    ...
    def __init__(
        ...
        self.param_history: Dict[str, List[float]] = {name: [] for name in self.trainable_params}

        initial_loss = self._compute_loss().item()
    ...
    def _setup_constraints(
        self, constraints: Optional[Dict[str, Tuple[float, float]]]
    ...
    def _setup_trainable_params(self) -> None:
        ...
        for name, value in self.initial_params.items():
            ...
            tensor = torch.tensor(
                value, dtype=DTYPE, device=self.device, requires_grad=not is_fixed
            )
    ...
    def optimize(
        ...
        for iteration in range(iterations):
            ...
            if callback:
                current_params = {
                    name: tensor.item() for name, tensor in self.trainable_params.items()
                }
