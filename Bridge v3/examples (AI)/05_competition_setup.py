...
def main():
    ...
    my_balsa = Custom(
        E=2.0e9,  # Young's modulus from bending tests
        sigma_compression=2.5e6,  # From compression test
        sigma_tension=8e6,  # From tension test
        density=220.0,  # Measured density of your sheets
    ...
    # Run optimization
    result = optimizer.optimize(iterations=5000, verbose=True, log_interval=1000)
    ...
    print(f"\n  PERFORMANCE:")
    print(
        f"    Critical Load:  {result.critical_load:.1f} N ({result.critical_load*0.2248:.1f} lbf)"
    )
