...
def main():
    ...
    print("  " + "-" * 65)
    print(
        f"  {'Material':<15} {'E (GPa)':>10} {'σ_c (MPa)':>12} {'σ_t (MPa)':>12} {'ρ (kg/m³)':>12}"
    )
    print("  " + "-" * 65)

    for name, mat in test_materials:
        print(
            f"  {name:<15} {mat.E/1e9:>10.1f} {mat.sigma_compression/1e6:>12.1f} "
    ...
    print("  " + "=" * 65)
    print(
        f"  {'Material':<15} {'Load (N)':>10} {'Weight (N)':>12} {'L/W Ratio':>12} {'Governing Mode':<20}"
    )
    print("  " + "-" * 65)
    ...
    for name, mat, result in results:
        print(
            f"  {name:<15} {result.critical_load:>10.1f} {result.weight:>12.4f} "
